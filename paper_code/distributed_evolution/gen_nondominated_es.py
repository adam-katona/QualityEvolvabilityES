...
class GenNonDominatedSortESOptimizer(GenESOptimizer):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)

    def update_population(self, descriptors, returns, novelty):
        self.optimizer.zero_grad()



        # The way the gradient calculation is implemented is to multiply the fitness with ratios, which multiplies by 1 on the forward() call
        # But multiplies with epsilon, and sigma and stuff on backward(), so in the end we get the gradient
        # The original gradient calculation is as follows:
        

        task_fitneses = np.array(returns)
        behaviour_characterization = np.array(novelty)

        evolvability = np.sum((behaviour_characterization - behaviour_characterization.mean(axis=0)) ** 2,axis=1)

        multi_objective_fitnesses = np.concatenate([task_fitneses.reshape(-1,1),evolvability.reshape(-1,1)],axis=1)
        # multi_objective_fitnesses shape is (pop_size,2)

        ranks,fronts = get_normalized_nondominated_ranks(multi_objective_fitnesses)

        ranks = torch.from_numpy(ranks).float()


        ratio = torch.cat(
            [self.population.ratio(descriptor) for descriptor in descriptors]
        )
        loss = -(ranks * ratio).mean()
        ...
        # save a bunch of stuff for later optional plotting / debugging
        self.last_behaviour_characterization = behaviour_characterization
        self.last_evolvability = evolvability
        self.last_task_fitneses = task_fitneses
        self.last_multi_objective_fitnesses = multi_objective_fitnesses
        self.last_fronts = fronts
